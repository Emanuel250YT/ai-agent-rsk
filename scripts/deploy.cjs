...
async function main() {
  ...
  if (!signers.length) {
    console.error(
      ...
      "Set DEPLOYER_PRIVATE_KEY in .env.local and retry.\n" +
      "Get testnet funds at: https://faucet.rootstock.io\n"
    );
    process.exit(1);
  }
  ...
  if (balance === 0n) {
    console.error(
      "\nx Deployer has no tRBTC. Get testnet funds at https://faucet.rootstock.io\n"
    );
    process.exit(1);
  }
